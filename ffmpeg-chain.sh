...
ffmpeg -threads 2 -thread_queue_size 10k -f rawvideo -pix_fmt yuv420p -s 720x406 -i - -i ffmpeg-logo.png -filter_complex "overlay=x='if(gte(t,0),-w+(mod(n,W+w))+5,NAN)':y=0" -f rawvideo -pix_fmt yuv420p -s 720x406 pipe:1 | \
#start 3rd ffmpeg to encode and push frame to ffserver
ffmpeg -thread_queue_size 10k -hwaccel auto -threads 2 -f rawvideo -pix_fmt yuv420p -s 720x406 -r 15 -i - -vcodec libx264 -crf 25 -movflags +faststart -tune zerolatency -profile:v baseline -maxrate 1M -s 720x406 -g 60 -r 15 -copytb 1 http://localhost:8090/feed1.ffm
